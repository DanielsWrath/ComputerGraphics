...
Material* Raytracer::parseMaterial(const YAML::Node& node)
{
    ...
    if(const YAML::Node* clr = node.FindValue("color")){
        *clr >> m->color;
        m->useTexture = false;
    }
    else if (const YAML::Node* txt = node.FindValue("texture")) {
        *txt >> m->texture;
        m->useTexture = true;
    }
    //node["color"] >> m->color;
...
Object* Raytracer::parseObject(const YAML::Node& node)
{
    ...
    if (objectType == "sphere") {
        Point pos;
        Triple radiusVec = Triple(0,0,0);
        double r, angle = 0;
        node["position"] >> pos;
        // TODO: possible to make radius (float,(float,float,float))
        node["radius"] >> r;
        if (const YAML::Node* rvec = node.FindValue("radiusVec")) {
            *rvec >> radiusVec;
        }
        if(const YAML::Node* ang = node.FindValue("angle")){
            *ang >> angle;
        }
        Sphere *sphere = new Sphere(pos,r, radiusVec, angle);
        returnObject = sphere;
    }
    ...
    if(objectType == "plane"){ 
		// added object (finite) plane, which is 4 points. 
		// NOTE: not a "real" plane. Use realPlane for that
    ...
    if (objectType == "cylinder") {
        // create cylinder object DOES NOT WORK
        Point pos0, pos1;
        double r;
        node["position0"] >> pos0;
        node["position1"] >> pos1;
        node["radius"] >> r;
        Cylinder *cylinder = new Cylinder(pos0, pos1, r);
        returnObject = cylinder;
    }
    // Create an object containing traingles using a file
    if (objectType == "object") {
        ...
        node["file"] >> file;
        node["position"] >> pos;
        char *fileChar = new char[file.length() + 1];
        strcpy(fileChar, file.c_str());
        model = glmReadOBJ(fileChar);
        Obj *obj = new Obj(model, pos);
        returnObject = obj;
        delete [] fileChar;
    }
    // Create a quad (two triangles) using 4 points
    if (objectType == "quad"){
		Point position0, position1, position2, position3;
        node["position0"] >> position0;
        node["position1"] >> position1;
        node["position2"] >> position2;
        node["position3"] >> position3;
        Quad *quad =  new Quad(position0, position1, position2, position3);
        returnObject = quad;
	}
	// construct a "real" plane. Uses point and normal vector
	if (objectType == "realPlane") {
		Point pos;
		Vector v;
		node["position"] >> pos;
		node["normal"] >> v;
		RealPlane *realplane = new RealPlane(pos, v);
		returnObject = realplane;
	}
    if (returnObject) {
			...
			if(const YAML::Node* render = doc.FindValue("RenderMode")){
				...
				*render >> m;
				if (m.compare("normal") == 0){
					...
					mode = 2;
                } else if (m.compare("gooch") == 0) {
                    mode = 3;
                }
			}
        
        //If rendermode is 3 (Gooch), parse parameters
            if (mode == 3){
                const YAML::Node* gooch = doc.FindValue("GoochParameters");
                parseGooch(*gooch);
